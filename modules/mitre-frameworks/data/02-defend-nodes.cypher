...
ON CREATE SET n.id = randomUUID()
SET n.description = "Issue a new credential to a user which supersedes their old credential.", n.name = "Reissue Credential", n.uri = "http://d3fend.mitre.org/ontologies/d3fend.owl#ReissueCredential";
...
MERGE (n:MitreDefendTechnique {d3fendId: "D3-UGPH"})
ON CREATE SET n.id = randomUUID()
...
ON CREATE SET n.id = randomUUID()
SET n.description = "Content Filtering techniques aid in the process of analyzing an input file for malicious or erroneous content and outputting a sanitized version.", n.name = "Content Filtering", n.uri = "http://d3fend.mitre.org/ontologies/d3fend.owl#ContentFiltering";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "Reads data in specified chunks or the contents of a specified file stored in the file device connected to the PC.", n.name = "OT Read File Command Event";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "An event in which a proximity sensor detects the presence of a nearby object without physical contact.", n.name = "Proximity Sensor Event";
...
MERGE (n:MitreDefendDigitalEventEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#Software-definedRadioWaveformApplicationLoadEvent"})
ON CREATE SET n.id = randomUUID()
...
ON CREATE SET n.id = randomUUID()
SET n.description = "A timer event involving a software timer managed by an operating system or application to schedule tasks, implement timeouts, or trigger periodic operations.", n.name = "Software Timer Event";
...
MERGE (n:MitreDefendDigitalEventEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#Software-definedRadioWaveformApplicationConfigurationEvent"})
ON CREATE SET n.id = randomUUID()
...
MERGE (n:MitreDefendNetworkNodeEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#AutomotiveElectronicControlUnit"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A vehicle computer embedded in a ground vehicle, executing real-time control logic over one or more vehicle subsystems and communicating with peer ECUs over an in-vehicle network such as CAN, CAN-FD, LIN, or automotive Ethernet.", n.name = "Automotive Electronic Control Unit";
...
MERGE (n:MitreDefendNetworkNodeEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#VehicleComputer"})
ON CREATE SET n.id = randomUUID()
SET n.description = "An embedded computing system within a vehicle that monitors, controls, or supports vehicle functions by processing data, communicating with other components, and interacting with sensors, actuators, and external interfaces.", n.name = "Vehicle Computer";
...
MERGE (n:MitreDefendNetworkNodeEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#SpacecraftComputer"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A vehicle computer installed on a spacecraft, executing real-time control or data-handling logic under spaceflight conditions (vacuum, radiation, extended communication latency) and communicating over buses such as MIL-STD-1553, SpaceWire, or CAN.", n.name = "Spacecraft Computer";
...
MERGE (n:MitreDefendNetworkNodeEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#ControlServer"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A controller that also acts as a server that hosts the control software that communicates with lower-level control devices, such as remote terminal units (RTUs) and programmable logic controllers (PLCs), over an OT network. In a SCADA system, this is often called a SCADA server, MTU, or supervisory controller.", n.name = "Control Server";
...
MERGE (n:MitreDefendNetworkNodeEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#AircraftComputer"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A vehicle computer installed on a fixed-wing or rotary-wing aircraft, executing real-time flight or mission logic and communicating over avionics buses such as ARINC 429, ARINC 664/AFDX, or MIL-STD-1553.", n.name = "Aircraft Computer";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "A physical link is a dedicated connection for communication that uses some physical media (electrical, electromagnetic, optical, to include clear spaces or vacuums.)  A physical link represents only a single hop (link) in any larger communications path, circuit, or network.  NOTE: not synonymous with data link as a data link can be over a telecommunications circuit, which may be a virtual circuit composed of multiple physical links.", n.name = "Physical Link";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "An OS API function that replaces the current process image with a new process image, executing a specified program.", n.name = "OS API Exec";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "A function which creates a new string based on a format specification and corresponding specified values.", n.name = "String Format Function";
...
MERGE (n:MitreDefendHardwareDeviceEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#Real-timeClock"})
ON CREATE SET n.id = randomUUID()
...
MERGE (n:MitreDefendSoftwareEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#VehicleControlSoftware"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A specialized subset of vehicle software strictly responsible for the direct management of the vehicle's physical state, orientation, and safety-critical dynamics. It functions as the logic layer between high-level mission objectives and the physical actuators.", n.name = "Vehicle Control Software";
...
MERGE (n:MitreDefendSoftwareEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#FlightSoftware"})
ON CREATE SET n.id = randomUUID()
SET n.description = "Flight Software (FSW) is software that runs on a processor embedded in a spacecraft's avionics. It is responsible for managing on-board activities, data processing and spacecraft health and safety. The name \"flight software\" reflects the location where it executes, i.e. in the spacecraft, to differentiate from \"ground software\", which runs in the ground segment.", n.name = "Flight Software";
...
MERGE (n:MitreDefendSoftwareEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#VehicleSoftware"})
ON CREATE SET n.id = randomUUID()
SET n.description = "Any software residing on a vehicle's onboard compute platforms.", n.name = "Vehicle Software";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "NTFS junction points are similar to NTFS symlinks but are defined only for directories. Only accepts local absolute paths.", n.name = "NTFS Junction Point";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "Reads data in specified chunks or the contents of a specified file stored in the file device connected to the PC.", n.name = "OT Read File Command";
...
ON CREATE SET n.id = randomUUID()
SET n.description = "The current functional state or control regime of a vehicle that determines how it is configured, how its systems behave, and what kind of operation it is performing at a given time.", n.name = "Vehicle Operating Mode";
...
MERGE (n:MitreDefendDigitalInformationEntity {uri: "http://d3fend.mitre.org/ontologies/d3fend.owl#WirelessCommand"})
ON CREATE SET n.id = randomUUID()
SET n.description = "A wireless command is a remote command transmitted to a system over a non-wired medium.", n.name = "Wireless Command";
