// Memgraph testcontainer harness for this module's ledger e2e.
//
// Deliberately a local copy of dt-ws's `test/integration/memgraph-container.ts` rather than
// an import: a published module must not take a test-time dependency on an app's internals,
// and the two runners differ (vitest here, jest there). The file is runner-agnostic — pure
// testcontainers plus the driver — so the copy carries no framework coupling.
//
