 ...
 * `deriveAnalysisClassId`) so the platform-side id matches what aegra
 * stores as `assistant_id`.
 */
...
/**
 * Derive an AnalysisClass id from a LangGraph graph name, matching aegra's
 * own `assistant_id = uuid5(ASSISTANT_NAMESPACE_UUID, graphName)` derivation.
 * Used by `DtLgModule.getAnalysisClasses()`.
 */
