...
import { z } from 'zod'
// zod v4 has built-in JSON Schema support via z.toJSONSchema()
...
const AttributesSchema = z.record(z.string(), z.any()).describe('Arbitrary key-value attributes defined by the element class')
...
const ConsolidatedAttributesFileSchema = z.object({
  boundaries: z.record(z.string(), ElementAttributesSchema).optional(),
  components: z.record(z.string(), ElementAttributesSchema).optional(),
  dataFlows: z.record(z.string(), ElementAttributesSchema).optional(),
  dataItems: z.record(z.string(), ElementAttributesSchema).optional()
