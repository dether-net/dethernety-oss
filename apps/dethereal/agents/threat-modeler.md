...
8. Invoke `Agent(infrastructure-scout-scoped)` in `discover elements` mode with the file allowlist (`scoped` plus, for each path, sibling files within one directory level so the scout can read companion files like `variables.tf` next to `rds.tf`). Before invoking it, remove from that allowlist every path in the Secret files class (see `infrastructure-scout.md`) and every file matched by `grep -lE '^\s*-?\s*kind:\s*"?Secret"?\s*$'`, and pass them to the scout as "present, not read". Receive `newElements` — the discovery-output schema array.
9. Compute the four-way delta locally by joining `priorElements_in_scope` and `newElements` on `suggestedName` (the same key the discovery agent uses):
   ...
   - **CHANGED-substrate** = name matches in both, but **either** `suggestedComponentType` differs (PROCESS↔STORE↔EXTERNAL_ENTITY transitions are always substrate flips) **or** `suggestedClass.id` differs (or one side has a class while the other doesn't). Substrate flips route through reclassification. *Caveat:* if both sides resolve to the same baseline (`dethernety-general`) class (the thin module case), class-id equality is not proof of substrate equivalence — when the prior element's class is from the baseline (`dethernety-general`) module, force `match_classes` against the new file content and use *its* output as the comparison source rather than treating equality as final.
