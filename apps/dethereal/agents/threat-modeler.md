...
10. Read crown-jewel flags for each REMOVED element from its attribute file (top-level `crown_jewel === true` or nested `attributes.crown_jewel === true`) so the pre-loop summary can call out crown-jewel-tagged removals.
