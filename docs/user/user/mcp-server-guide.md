...
**Dethereal** is Dethernety's Model Context Protocol (MCP) server, enabling AI assistants like Claude to directly interact with the Dethernety platform. This allows you to create, validate, import, export, and manage threat models through natural language conversations.

### What is MCP?

The [Model Context Protocol](https://modelcontextprotocol.io/) is an open standard that allows AI assistants to interact with external tools and services. Dethereal implements MCP to expose Dethernety's threat modeling capabilities as tools that AI can use.

### Key Capabilities

| Capability | Description |
|------------|-------------|
| **Create Models** | Build threat models from natural language descriptions |
| **Import/Export** | Transfer models between files and the platform |
| **Validate** | Check model structure before importing |
| **Browse Classes** | Discover available component types, controls, and data classes |
| **Update Models** | Modify existing models programmatically |
...
The AI will use the `login` tool, which opens your browser to the Cognito login page. After authentication, tokens are cached locally at `~/.dethernety/tokens.json`.
...
Authenticates via browser-based OAuth. Opens your default browser to the Cognito login page.
...
```
Create a threat model for an e-commerce platform with:
- Customer-facing web application
- Mobile app
- REST API gateway
- Microservices: catalog, orders, payments, users
- PostgreSQL and Redis databases
- Message queue for async processing

Place it in the "E-Commerce" folder.
```

### Exporting for Review

```
Export the threat model with ID "abc123" to ./my-model/ for review
```

### Validating Before Import

```
Validate the model in ./updated-model/ before importing
```

### Updating an Existing Model

```
Update model "abc123" from the files in ./my-model/
Keep any elements that aren't in the update files.
```

### Finding Available Classes

```
What component classes are available for web servers?
```

```
Show me all control classes related to encryption
```
...
- [AI-Powered Workflows](ai-powered-workflows.md) - Using AI in the Dethernety UI
- [Building Your First Model](building-your-first-model.md) - Manual model creation
- [Understanding Modules](understanding-modules.md) - Module system concepts
