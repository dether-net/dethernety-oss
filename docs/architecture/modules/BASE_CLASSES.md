...
The production-ready base class that stores class definitions in the graph database (Bolt/Cypher compatible, e.g., Neo4j, Memgraph) and evaluates Rego policies using an OPA server.
...
- `driver` - Graph database driver (for `DbOps` instance lookups)
...
```typescript
getMetadata(): Promise<DTMetadata>
```

**Flow:**
1. Read `metadata.json` from module directory
2. Scan each class type directory (`ComponentClasses/`, etc.)
3. For each class subdirectory, read `metadata.json`
4. Install Rego policies via `OpaOps.installPolicies()`
...
class DevModule extends DtFileOpaModule {
  constructor(driver: any) {
    super('./modules', 'dev-module', driver);
  }
}
```
...
class EmbeddedModule extends DtFileJsonModule {
  constructor(driver: any) {
    super('./modules', 'embedded-module', driver);
  }
}
```
...
Base class for LangGraph-integrated AI analysis modules.
...
class AttackScenarioModule extends DtLgModule {
  constructor(driver: any, logger: Logger) {
    super('dethermine-attack-scenario', driver, logger, {
