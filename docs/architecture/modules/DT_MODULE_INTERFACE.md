...
export interface DTModule {
  ...
  // Optional - Analysis methods (for LangGraph modules)
  runAnalysis?(
    id: string,
...
export interface AnalysisClassMetadata {
  id?: string;              // Maps to LangGraph assistant_id
  name: string;             // Analysis name (maps to LangGraph graph name)
...
Starts a LangGraph analysis workflow.
...
- `analysisClassId` - The analysis class (LangGraph assistant) ID
...
Starts an interactive chat session with a LangGraph analysis.
