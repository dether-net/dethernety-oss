...
| [`CONSOLE.md`](./CONSOLE.md) | The operator console — one binary with an `init` one-shot and a `daemon`, the state file, the mode layer and the one narrow rewrite of who may sign in, content mounts, entitled-artifact installs, its authentication posture, and why it has no process control. |
