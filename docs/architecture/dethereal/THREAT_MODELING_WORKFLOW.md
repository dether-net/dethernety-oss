...
**Trust skeleton (zone + plane).** Beyond enforcement, Phase 4 ratifies each *leaf* boundary's **trust zone** (`UNTRUSTED`/`PUBLIC`/`EXPOSED`/`INTERNAL`/`RESTRICTED`/`VENDOR`; `null` = inherit) and **operational plane** (`WORKLOAD`/`MANAGEMENT`) in one batched gate, computed by `validate_model_json(action: 'zoning', assets: 'skeleton')` (`RESTRICTED` is deferred to the close of Phase 7). A boundary that only *nests other boundaries* is a **structural container** — it has no single well-defined zone and abstains (rendered `— structural`, not proposed a zone). Identity / compute-node / location / business-function grouping belongs in `domains`/`planes` **tags**, not zone-bearing boundaries. Conduits (approved channels) are ratified in Phase 5, and the `RESTRICTED` promotion + advisory zoning-coherence findings land at Phase 7 / Phase 9. Full mechanics: [TRUST_ZONING.md](TRUST_ZONING.md).
