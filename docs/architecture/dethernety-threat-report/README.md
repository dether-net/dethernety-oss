...
The **Dethernety Threat Report** is a pure, read-only reporting module layered over an existing threat model. It mounts through the platform's native analysis lifecycle — no `dt-ui`, `dt-ws`, or `dt-core` changes — and computes a point-in-time **snapshot** of the model when an analyst runs it, persisting that snapshot on the standing `Analysis` node. Opening the results renders a single Vue application over that snapshot, presenting six analyst-facing surfaces: a posture roll-up, a MITRE ATT&CK coverage matrix, flow-route reachability, structural boundary crossings, the residual-risk findings ledger, and a per-element drill-down. The report is engineered to under-claim: at every step it prefers an honest gap to a flattering aggregate.
