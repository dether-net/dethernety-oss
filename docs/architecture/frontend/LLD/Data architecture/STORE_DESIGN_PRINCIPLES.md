...
This document outlines the design principles, patterns, and best practices for creating production-ready Pinia stores. These guidelines ensure consistency, maintainability, type safety, and enterprise-grade reliability across all store implementations.
...
Not every store needs the full enterprise pattern suite. Choose complexity based on your store's requirements:

#### **Simple Pattern (UI State, Flags, Basic Data)**
```typescript
// ✅ GOOD: Simple store for UI state
...
```typescript
// ✅ GOOD: Store with API integration and moderate complexity
...
#### **Enterprise Pattern (Critical Systems, External Dependencies)**
```typescript
// ✅ GOOD: Full enterprise pattern for critical authentication store
...
| Store Type | Simple | Intermediate | Enterprise |
|------------|--------|-------------|------------|
| UI State | ✅ | ❌ | ❌ |
| Local Data Cache | ✅ | ✅ | ❌ |
| API Integration | ❌ | ✅ | ✅ |
...
// Stage 3: Full enterprise pattern when critical
const createStore = (config: StoreConfig = {}) => {
...
// ❌ BAD: Inferred return types for public methods
...
```typescript
// ✅ GOOD: Configurable data extraction
...
// ❌ BAD: Hard-coded provider-specific logic
...
```typescript
// ✅ GOOD: Meaningful computed properties
...
```typescript
// ✅ GOOD: Comprehensive state reset
...
```typescript
// ✅ GOOD: Abstract critical dependencies
...
```typescript
// ✅ GOOD: Direct usage for simple, stable utilities
...
```typescript
// ✅ GOOD: Comprehensive dependency injection
...
```typescript
// ✅ GOOD: Structured GraphQL integration
...
```typescript
// ✅ GOOD: Library-specific error handling
...
```typescript
// ✅ GOOD: Conditional imports and tree shaking
...
// ❌ BAD: Accessing stores inside functions without declaration
const useOrderStore = defineStore('orders', () => {
  const createOrder = async (orderData: CreateOrderRequest) => {
...
```typescript
// ✅ GOOD: Composable store features
...
```typescript
// ✅ GOOD: Cross-store business rule validation
...
```typescript
// ✅ GOOD: Promise-based concurrency control
...
// ❌ BAD: Boolean-based mutex (race conditions possible)
...
```typescript
// ✅ GOOD: Request deduplication by key
...
// ❌ BAD: Always bundled heavy features
...
```typescript
// ✅ GOOD: Production-ready caching with memory management
class LRUCache<T> {
...
```typescript
// ✅ GOOD: Memory-safe request deduplication
...
```typescript
// ✅ GOOD: Smart retry logic with different strategies for different errors
...
```typescript
// ✅ GOOD: Optimized reactivity choices
...
```typescript
// ✅ GOOD: Efficient computed properties
...
```typescript
// ✅ GOOD: Handling reactivity edge cases
export const useEdgeCaseStore = defineStore('edgeCase', () => {
  ...
  watchEffect(() => {
    ...
    logResult(result) // ✅ GOOD: Side effect in watchEffect
  })
...
// ❌ BAD: Reactive expensive operations
...
```typescript
// ✅ GOOD: Separable concerns for testing
...
- [ ] Appropriate complexity level for requirements (simple/intermediate/enterprise)
...
These patterns and principles ensure that Pinia stores are:
...
Follow these guidelines to create stores that are robust, scalable, and maintainable across your application ecosystem.
