...
const updateItemName = (itemId: string, newName: string) => {
  ...
  if (item) {
    item.name = newName // Direct mutation - BAD!
  }
}
...
// ✅ BETTER: Using structuredClone for deep immutability
const updateItemName = (itemId: string, newName: string) => {
  const index = items.value.findIndex(i => i.id === itemId)
  if (index !== -1) {
...
This comprehensive documentation covers all the key implementation patterns used in your architecture. Each pattern is designed to work together to create a robust, maintainable, and production-ready application.
